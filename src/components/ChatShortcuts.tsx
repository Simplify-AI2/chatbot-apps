...
const ChatShortcuts: React.FC = () => {
  ...
  const loadChatSettings = async () => {
    const filteredAndSortedChatSettings = await chatSettingsDB.chatSettings
        ...
        .sortBy('name');
    // Hide Math Tutor from sidebar
    setChatSettings(filteredAndSortedChatSettings.filter(setting => setting.name !== 'Math Tutor'));
  };
  ...
  return (
      <div>
        {/* Hide Math Tutor from sidebar options */}
        {chatSettings.filter(setting => setting.name !== 'Math Tutor').map((setting) => (
